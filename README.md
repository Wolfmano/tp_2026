...
* Каждая работа должна быть в отдельном каталоге с соответствующим названием:
  ...
  * `T4` - задание 1, геометрические фигуры
  * `T2` - задание 2, потоки и итераторы (номер 2 - не опечатка)
  * `T3` - задание 3, алгоритмы
  * 'F0' - РГР (курсовая работа)
...
```
  git remote add upstream https://github.com/alexei-tchervinsky/tp_2025.git
```
...
```
  git checkout main
```
2. Создать новую ветку с необходимым названием от main-ветки в **локальном** репозитории и переключиться на нёё:
```
...
После отправки готовой работы в свой удалённый репозиторий (fork), необходимо отправить её на проверку в основной репозиторий. Для этого средствами GitHub создайте pull request из соответствующей ветки fork'а в main ветку основного репозитория.

Также для проверки выполнения тестов можно создать pull request в main ветку fork'а, **но принимать (merge) этот pull request не следует**! Для активации тестов необходимо нажать кнопку "Enable..." на странице Actions.

### Синхронизация с основным репозиторием

Работы должны сдаваться по порядку. После того, как ваш pull request принят (merge) в основном репозитории, необходимо синхронизировать main-ветки fork и основного репозитория:
1. Переключиться на main-ветку в **локальном** репозитории:
```
  git checkout main
```
...
```
  git fetch upstream
```
3. Объединить изменения из main **основного** репозитория в main **локального**:
```
  git merge upstream/main
```
4. Отправить изменения main **локального** репозитория в **свой удалённый (fork)**:
```
  git push
```
Если в этот момент вы уже выполняете следующую работу в другой ветке, то необходимо добавить в неё изменения из **локальной** ветки main:

5. Переключиться на ветку с лабораторной:
```
  git checkout -b ivanov.ivan/T2
```
6. Синхронизировать рабочую ветку с **локальной** веткой main
```
  git merge main
```
